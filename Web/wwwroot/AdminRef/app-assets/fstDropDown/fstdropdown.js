...
function setFstDropdown() {
    ...
    function changeMultipleSelect(select, event, dd, selectAll, opened) {
        ...
        if (opened)
            dd.querySelector(".fstselected").textContent = length == 1 ? event.target.textContent : length + " «‰ Œ«» ‘œÂ";
        selectAll.textContent = selectAll.selected ? "Deselect All" : "«‰ Œ«» Â„Â";
    }
    ...
    function selectAllOptions(event) {
        ...
        initNewEvent("change", select);
        event.target.textContent = !selected ? "«‰ Œ«» Â„Â" : "Deselect All";
